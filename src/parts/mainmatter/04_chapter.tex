...
\begin{figure}[h]
	...
	\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
		...
		\node[left of=I-2, rotate=90, xshift=-.5cm, yshift=-1cm] {observation: $\vec{o}_k$};
		...
		\node[annot,right of=H4, node distance=2.5cm] (act) {action: $\alpha_k$};
		...
		\node[annot,above of=H3-1, node distance=.8cm] {$\mu_k$};
		\node[annot,below of=H3-2, node distance=.8cm] {$\log(\sigma_k)$};
		\node[annot,above of=H4, node distance=.8cm] {$\mathcal{N}(\mu_k,\sigma^2_k)$};
