...
\begin{table}[h]
	\begin{center}
		\begin{tabular}{r|c c c c}
			...
			iterations (-) & - & 18 & 23 & 20 \\
			calculation time (h:min) & 0:15 & 2:01 & 1:19 & 1:02 \\
...
\pgfplotstableread[col sep=comma] {\detokenize{./src/pics/tikz/dat-no99-i0-500m-dt250ms-data-from-no92.dat}}\dataVIfiveH
...
Figure \ref{tikz:vi1000m} shows in blue the trajectory in scenario 2 after 43 value iterations. The thick line is the glider trajectory $[x(t),z(t)]^T$, the thin line is the control sequence $[x(t),\alpha(t)]^T$ The agent is dropped at the start state with zero velocity. As can be seen, both the policy from VI and the optimal control (red) direct the agent towards the target.
