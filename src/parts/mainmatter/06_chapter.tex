...
To see if TRPO spoils the policy's ability to find the goal from differing initial states, the agent is launched from other states than before and the results before and after TRPO are compared. The policy used for the comparison is the same as in Fig. \ref{tikz:vi1000m}. Figures \ref{tikz:vi_result_before_trpo} and \ref{tikz:vi_result_before_trpo_2} show how the agent behaves when confronted with varying initial states. Before TRPO, he reaches the goal from all tested initial states.
...
%	\end{axis}
	...
	\end{tikzpicture}	
	\caption{value iteration - trajectories from various initial states with zero velocity}
